...
def shade(
    ...
    mask_val, _ = interpolate(vert_mask[None, :, None], rast, t_pos_idx.int())
    #return torch.where(fragment_mask, out, torch.zeros_like(out))
    #expland the last dim by 3
    mask_val = torch.cat([mask_val]*3, dim=-1)
    return torch.cat((mask_val, alpha), dim=-1)
...
def render_layer(
    ...
    color = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, 
        view_pos, light_pos, light_power, mesh.material, min_roughness, vert_mask, mesh.t_pos_idx, rast_out_s)
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), [resolution*spp, resolution*spp]) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, light_pos, light_power, resolution, min_roughness, spp, msaa, vert_mask), rast)]
    if return_rast_map:
        return rast.detach()
    #clear bg color to black 
