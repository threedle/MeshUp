...
# Returns a batch of camera parameters that anchors around the given elev and azim
class CameraBatch_Anchor(torch.utils.data.Dataset):
    def __init__(
        self,
        image_resolution,
        distances,
        ...
        self.dist_min = distances[0]
        self.dist_max = distances[1]
        self.anchor_azimuth = anchor_azimuth
        self.anchor_elevation = anchor_elevation
        self.anchor_dist = anchor_dist
        self.azim_std = azim_std 
        self.elev_std = elev_std
        self.dist_std = dist_std

        self.azim_min = azimuths[0]
        self.azim_max = azimuths[1]
        self.elev_min = elevations[0]
        self.elev_max  = elevations[1]
        self.fov_min = fovs[0]
        self.fov_max = fovs[1]
    ...
    def __getitem__(self, index):
        #if the anchor azimuth is None than sample from uniform distribution
        if self.anchor_azimuth is None:
            azim = np.radians( np.random.uniform( self.azim_min, self.azim_max+1.0 ) )
        else: 
            azim = np.radians( np.random.normal(self.anchor_azimuth, self.azim_std))
        #if the anochor azimuth is None than sample from uniform distribution
        if self.anchor_elevation is None:
            elev = np.radians( np.random.uniform( 0, self.elev_max ))
        else:
            elev = np.radians( np.random.normal(self.anchor_elevation, self.elev_std))
        if self.anchor_dist is None: 
            dist = np.random.uniform( self.dist_min, self.dist_max )
        else: 
            dist = np.random.normal(self.anchor_dist, self.dist_std)
        fov = np.random.uniform( self.fov_min, self.fov_max )
        proj_mtx = persp_proj(fov)
...
# Returns a batch of camera parameters that anchors around the given elev and azim
class CameraBatch_Single(torch.utils.data.Dataset):
    def __init__(
        self,
        image_resolution,
        azimuth,
